 ...
 * 3. En la barra superior, asegúrate de que esté seleccionada la función "inicializarBaseDeDatos".
 * 4. Haz clic en "Ejecutar". Esto creará automáticamente todas las tablas (hojas)
 *    con sus columnas formateadas, colores y datos iniciales.
 * 5. Haz clic en el botón azul superior "Implementar" > "Nueva implementación".
 * 6. Tipo: "Aplicación web".
 *    - Ejecutar como: "Yo" (tu correo de Google).
 *    - Quién tiene acceso: "Cualquiera" (IMPORTANTE para que Vercel y tu Web App puedan comunicarse).
 * 7. Copia la URL generada (termina en /exec) y pégala en la Web App.
 * =========================================================================
 */

// DEFINICIÓN DE HOJAS Y SUS ENCABEZADOS EXACTOS
var SCHEMA = {
  PACIENTES: {
    name: 'Pacientes',
    headers: [
...
/**
 * Agrega un menú personalizado en la barra de Google Sheets al abrir la hoja.
 */
function onOpen() {
...
/**
 * FUNCIÓN PRINCIPAL QUE CREA TODAS LAS HOJAS Y SUS TABLAS AUTOMÁTICAMENTE
 * Puedes ejecutarla directamente desde el editor de Google Apps Script haciendo clic en "Ejecutar".
 */
function inicializarBaseDeDatos() {
  ...
  for (var key in SCHEMA) {
    ...
    // Configurar encabezados si la hoja está vacía o si la primera fila no coincide
    var lastRow = sheet.getLastRow();
    if (lastRow === 0) {
      sheet.appendRow(table.headers);
      formatearEncabezado(sheet, table.headers.length);
    } else {
      // Si ya tiene encabezados, asegurarse de que el formato esté elegante
      formatearEncabezado(sheet, table.headers.length);
    }
  }

  // Eliminar la hoja "Hoja 1" o "Sheet1" predeterminada si ya existen nuestras tablas
  try {
    ...
    // Si no se puede borrar, no afecta
  }

  // Poblar con registros iniciales si las tablas están vacías
  sembrarDatosIniciales(ss);

  Logger.log('Inicialización completada. Tablas creadas: ' + createdCount + ', Tablas existentes: ' + existingCount);
  
  try {
    SpreadsheetApp.getActiveSpreadsheet().toast(
      '✅ Base de datos configurada correctamente con ' + Object.keys(SCHEMA).length + ' tablas.',
      'Dr. Belleza Sistema',
      8
    );
  } catch (e) {
    // En ejecución directa de consola toast no bloquea
  }

  return {
    status: 'ok',
    message: 'Base de datos inicializada correctamente',
    tables: Object.keys(SCHEMA).map(function(k) { return SCHEMA[k].name; })
  };
}

/**
 * Da formato visual profesional a los encabezados
 */
function formatearEncabezado(sheet, columnCount) {
  var headerRange = sheet.getRange(1, 1, 1, columnCount);
  headerRange.setBackground('#065f46'); // Verde Esmeralda Dr. Belleza
...
/**
 * Inserta los usuarios y procedimientos iniciales si las tablas están vacías
 */
function sembrarDatosIniciales(ss) {
  ...
  if (userSheet && userSheet.getLastRow() <= 1) {
    userSheet.appendRow([
      ...
      'true',
      '2026-01-01',
      'Super Administrador Permanente'
    ]);
    userSheet.appendRow([
      'USR-101',
      'Dr. Jorge Apelencia',
      'jorge.apelencia@drbelleza.com',
      'super_admin',
      '+5491145678900',
      'jorge2026',
      'true',
  ...
  if (procSheet && procSheet.getLastRow() <= 1) {
    ...
    procSheet.appendRow(['PRC-003', 'QX-MAMA', 'Mamoplastia de Aumento Dual-Plane', 'Corporal', 3800, 150, 'true', 60, 'true']);
    procSheet.appendRow(['PRC-004', 'EST-BOTOX', 'Toxina Botulínica Full Face (Botox)', 'Medicina Estética', 350, 45, 'false', 50, 'true']);
  }

  // 3. Planes de financiamiento iniciales
  ...
  if (planSheet && planSheet.getLastRow() <= 1) {
    ...
    planSheet.appendRow(['PLAN-002', 'Plan 12 Meses - Mensual (8% Recargo)', 12, 'Mensual', 12, 8, 25, 'true']);
    planSheet.appendRow(['PLAN-006', 'Pago Contado / En Una Sola Cuota', 1, 'Mensual', 1, 0, 100, 'true']);
  }
}

/**
 * ENDPOINT GET (Para pruebas en navegador y pings)
 */
function doGet(e) {
  var ss = SpreadsheetApp.getActiveSpreadsheet();
  ...
  return responderJSON({
    status: 'ok',
    message: 'Servicio Web App Dr. Belleza activo y en funcionamiento',
...
/**
 * ENDPOINT POST: RECIBE TODAS LAS ACCIONES CRUD DE LA WEB APP
 * Usa LockService para prevenir concurrencia y choques de escritura.
 */
function doPost(e) {
  var lock = LockService.getScriptLock();
  try {
    // Espera hasta 30 segundos si otro usuario está escribiendo al mismo tiempo
    lock.waitLock(30000);
    ...
    if (!e || !e.postData || !e.postData.contents) {
      return responderJSON({ status: 'error', message: 'No se recibieron datos en la solicitud' });
    }
    ...
    switch (action) {
      ...
      case 'DELETE_PATIENT':
        // BORRADO EN CASCADA: Paciente + Abonos + Reintegros + Recordatorios
...
/**
 * Helper para responder siempre JSON con los headers CORS adecuados
 */
...
/**
 * OBTENER TODOS LOS DATOS PARA LA WEB APP
 */
function obtenerTodosLosDatos(ss) {
  var patients = leerHojaComoObjetos(ss.getSheetByName(SCHEMA.PACIENTES.name), mapearPacienteDesdeFila);
  var payments = leerHojaComoObjetos(ss.getSheetByName(SCHEMA.ABONOS.name), mapearPagoDesdeFila);
  var refunds = leerHojaComoObjetos(ss.getSheetByName(SCHEMA.REINTEGROS.name), mapearReintegroDesdeFila);
  var users = leerHojaComoObjetos(ss.getSheetByName(SCHEMA.USUARIOS.name), mapearUsuarioDesdeFila);
  var crmEvents = leerHojaComoObjetos(ss.getSheetByName(SCHEMA.CRM.name), mapearEventoCRMDesdeFila);
  var procedures = leerHojaComoObjetos(ss.getSheetByName(SCHEMA.PROCEDIMIENTOS.name), mapearProcedimientoDesdeFila);
  var financingPlans = leerHojaComoObjetos(ss.getSheetByName(SCHEMA.PLANES.name), mapearPlanDesdeFila);
...
function leerHojaComoObjetos(sheet, mapperFn) {
  ...
  var list = [];
  // Empezar en la fila 1 (la 0 son encabezados)
...
// MAPEOS DE FILAS A OBJETOS TYPESCRIPT
...
/**
 * BORRAR PACIENTE EN CASCADA (ELIMINA PACIENTE + ABONOS + REINTEGROS + CRM)
 */
function borrarPacienteEnCascada(ss, patientId) {
  if (!patientId) throw new Error('ID de paciente no provisto');

  var idStr = String(patientId);

  // 1. Borrar de Pacientes
  var patSheet = ss.getSheetByName(SCHEMA.PACIENTES.name);
  borrarFilasDondeColumnaSea(patSheet, 0, idStr);

  // 2. Borrar de Abonos (columna 1 es ID Paciente)
  var paySheet = ss.getSheetByName(SCHEMA.ABONOS.name);
  var deletedPayments = borrarFilasDondeColumnaSea(paySheet, 1, idStr);

  // 3. Borrar de Reintegros (columna 1 es ID Paciente)
  var refSheet = ss.getSheetByName(SCHEMA.REINTEGROS.name);
  var deletedRefunds = borrarFilasDondeColumnaSea(refSheet, 1, idStr);

  // 4. Borrar de CRM (columna 1 es ID Paciente)
  var crmSheet = ss.getSheetByName(SCHEMA.CRM.name);
  var deletedEvents = borrarFilasDondeColumnaSea(crmSheet, 1, idStr);

  return {
    status: 'ok',
    message: 'Paciente y todos sus registros asociados eliminados en cascada',
    patientId: patientId,
...
function borrarFilasDondeColumnaSea(sheet, colIndex, targetValue) {
  ...
  var values = sheet.getDataRange().getValues();
  var count = 0;
  // Recorrer de abajo hacia arriba para no alterar los índices de filas
...
/**
 * GUARDAR PAGO Y ACTUALIZAR SALDO EN HOJA PACIENTES
 */
function guardarPagoYActualizarSaldo(ss, payment) {
  if (!payment || !payment.id) throw new Error('Datos de pago inválidos');
  ...
  // Recalcular saldo en hoja de Pacientes
  recalcularTotalesPaciente(ss, payment.patientId);

  return { status: 'ok', message: 'Pago registrado y saldo actualizado', id: payment.id };
}
...
function borrarPagoYRecalcular(ss, paymentId) {
  ...
  if (patientId) {
    recalcularTotalesPaciente(ss, patientId);
  }
...
function guardarReintegro(ss, refund) {
  if (!refund || !refund.id) throw new Error('Datos de reintegro inválidos');
  ...
  return { status: 'ok', message: 'Reintegro guardado', id: refund.id };
}

function borrarReintegro(ss, refundId) {
  var refSheet = ss.getSheetByName(SCHEMA.REINTEGROS.name);
  borrarFilasDondeColumnaSea(refSheet, 0, String(refundId));
  ...
  if (patSheet && patSheet.getLastRow() > 1) {
    ...
    for (var j = 1; j < patValues.length; j++) {
      if (String(patValues[j][0]) === idStr) {
        ...
        patSheet.getRange(j + 1, 11).setValue(totalPaid); // Col K: Total Abonado
        patSheet.getRange(j + 1, 12).setValue(newBalance); // Col L: Saldo Pendiente
        patSheet.getRange(j + 1, 14).setValue(newStatus); // Col N: Estado
        patSheet.getRange(j + 1, 18).setValue(new Date().toISOString()); // Col R: Actualizado
        break;
      }
    }
  }
...
function borrarUsuario(ss, userId) {
  ...
  for (var i = 1; i < values.length; i++) {
    if (String(values[i][0]) === String(userId)) {
      // Proteger usuario inmutable
      if (String(values[i][7]).toLowerCase() === 'true') {
        throw new Error('Este usuario tiene protección permanente de inmutabilidad y no puede ser eliminado.');
      }
      sheet.deleteRow(i + 1);
...
function guardarEventoCRM(ss, ev) {
  ...
  return { status: 'ok', message: 'Evento de CRM guardado' };
}

/**
 * SINCRONIZACIÓN MASIVA (SUBIDA COMPLETA DESDE LA WEB APP)
 */
